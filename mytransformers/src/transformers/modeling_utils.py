...
class ModuleUtilsMixin:
    ...
    def select_attention_mask_dim(self, encoder_attention_mask: Tensor) -> Tensor:
        """
        Select an attention mask dim (e.g., (Batch_size x seq_length x seq_length) -> (Batch_size x 1 x seq_length))

        Args:
            encoder_attention_mask (:obj: `torch.Tensor`): An attention mask.

        Returns:
        :obj:`torch.Tensor`: The processed attention mask
        """
        if encoder_attention_mask.dim() == 3:
            encoder_attention_mask = encoder_attention_mask.select(1, torch.tensor(0))[:, None, :]
        elif encoder_attention_mask.dim() == 2:
            encoder_attention_mask = encoder_attention_mask
        else:
            raise ValueError(
                f"{encoder_attention_mask} not recognized. `dim` should be set to either `3` or `2`"
            )
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor, input_shape: Tuple[int], device: device, three_dim_attention_mask: bool()) -> Tensor:
        """
        ...
        """
        #print("get_extended_attention_mask ModuleUtilsMixin in modeling_utils L.234 three_dim_attention_mask",str(three_dim_attention_mask))
        #print("get_extended_attention_mask ModuleUtilsMixin in modeling_utils L.234 self.config.is_decoder",str(self.config.is_decoder))
        #print("get_extended_attention_mask ModuleUtilsMixin in modeling_utils L.234 attention_mask",str(attention_mask.shape))
        if three_dim_attention_mask==True:
            # conduct attention mask below for graph
            ...
            if attention_mask.dim() == 3:
                ...
                #     extended_attention_mask = causal_mask[:, None, :, :] * attention_mask[:, None, :, :]
                # else:
                # Provided a padding mask of dimensions [batch_size, seq_length, seq_length]
            ...
            if attention_mask.dim() == 3:
                ...
                if self.config.is_decoder:
                    ...
                    extended_attention_mask = attention_mask[:, None, None, :]
                    # matrix_attention_mask_list = []
                    # for i in range(batch_size):
                    #     for index in range(len(attention_list)-1):
                    #         matrix_attention_mask = torch.block_diag(torch.ones(attention_mask[i][index][0],attention_mask[i][index][1])) if index==0 else torch.block_diag(matrix_attention_mask,torch.ones(attention_mask[i][index][0],attention_mask[i][index][1]))
                    #     matrix_attention_mask = torch.block_diag(matrix_attention_mask, torch.zeros(attention_mask[i][len(attention_mask[i])-1][0],attention_mask[i][len(attention_mask[i])-1][1]))
                    #     matrix_attention_mask_list.append(matrix_attention_mask)
                    # extended_attention_mask = torch.stack(matrix_attention_mask_list, dim=0)[:, None, :, :]
            else:
                raise ValueError(
                    f"Wrong shape for input_ids (shape {input_shape}) or attention_mask (shape {attention_mask.shape})"
                )
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def __init__(self, config: PretrainedConfig, *inputs, **kwargs):
        ...
        self.config = config
        #print("PretrainedModel L.493 self.config",str(self.config))
