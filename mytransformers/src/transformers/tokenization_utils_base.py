...
class PreTrainedTokenizerBase(SpecialTokensMixin):
    ...
    def pad(
        ...
        # e.g. PaddingStrategy.MAX_LENGTH
        padding_strategy, _, max_length, _ = self._get_padding_truncation_strategies(
            padding=padding, max_length=max_length, verbose=verbose
        )

        required_input = encoded_inputs[self.model_input_names[0]]
        # encoded_inputs -> {'input_ids': [[32103, 1051, 5901, 32104, 16, 32105, 1826, 32100, 32103, 1051, 5901, 32104, 2074, 32105, 9455, 22671, 1]]}, 
        # required_input -> [[32103, 1051, 5901, 32104, 16, 32105, 1826, 32100, 32103, 1051, 5901, 32104, 2074, 32105, 9455, 22671, 1]]
        if required_input and not isinstance(required_input[0], (list, tuple)):
            ...
            print("encoded_inputs",str(encoded_inputs))
        ...
        for i in range(batch_size):
            # inputs -> {'input_ids': [32103, 1051, 5901, 32104, 16, 32105, 1826, 32100, 32103, 1051, 5901, 32104, 2074, 32105, 9455, 22671, 1]}
            inputs = dict((k, v[i]) for k, v in encoded_inputs.items())

            # outputs -> {'input_ids': [32103, 1051, 5901, 32104, 16, 32105, 1826, 32100, 32103, 1051, 5901, 32104, 2074, 32105, 9455, 22671, 1, 0, 0, 0, .....
            outputs = self._pad(
                inputs,
                max_length=max_length,
                padding_strategy=padding_strategy,
            ...
            # key -> input_ids value -> attention_mask
            # batch_outputs -> {'input_ids': [[32103, 1051, 5901, 32104, 16, 32105, 1826, 32100, 32103, 1051, 5901, 32104, 2074, 32105, 9455, 22671, 1, 0, 0....0]], 'attention_mask': [[(1, 17), (9, 9), (495, 495)]]}
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        # batch_outputs -> batch_outputs {'input_ids': [32103, 1051, 5901, 32104, 16, 32105, 1826, 32100, 32103, 1051, 5901, 32104, 2074, 32105, 9455, 22671, 1]}
    ...
    def _create_attention_mask(
        ...
        # matrix_mask = np.zeros(0)
        # zero_mask_matrix = np.zeros(0)
        # one_mask_matrix = np.zeros(0)
        # cat_matrix = np.zeros(0)
        # matrix_attention_mask = np.zeros(0)
        # # counter_mask = index - previous_counter - 1
        # previous_counter = -1
        # index = 0
        # id = 0
        # for index, id in enumerate(required_input):
        #     # if token=="<sep>" then append count for masking ids
        #     if id==32100:
        #         if matrix_mask.size==0:
        #             one_mask_matrix = np.ones((index - previous_counter,len(required_input)), dtype=np.int32)
        #             zero_mask_matrix = np.zeros((index - previous_counter, difference), dtype=np.int32)
        #             matrix_mask = np.concatenate((one_mask_matrix, zero_mask_matrix),1, dtype=np.int32)
        #         else:
        #             # concatenate (previous_mask, current_mask, zero_mask) Size: length_mask_row x length_mask_column

        #             matrix_mask = np.concatenate((matrix_mask, np.concatenate((np.zeros((index - previous_counter, previous_counter+1),dtype=np.int32),np.ones((index - previous_counter, index - previous_counter), dtype=np.int32),np.zeros((index - previous_counter, (len(required_input) - index - 1)), dtype=np.int32), np.zeros((index - previous_counter, difference), dtype=np.int32)),1, dtype=np.int32)),0, dtype=np.int32)
        #         previous_counter = index
        # if previous_counter >= 0:
        #     zero_mask_matrix = np.zeros((index - previous_counter,previous_counter+1), dtype=np.int32)
        #     one_mask_matrix = np.ones((index - previous_counter,(len(required_input)-previous_counter-1)), dtype=np.int32)
        #     cat_matrix = np.concatenate((zero_mask_matrix, one_mask_matrix, np.zeros((index - previous_counter , difference), dtype=np.int32)),1, dtype=np.int32 )
        #     matrix_mask = np.concatenate((matrix_mask, cat_matrix),0, dtype=np.int32)
        #     zero_mask_matrix = np.zeros((difference, max_length), dtype=np.int32)
        #     matrix_attention_mask = np.concatenate((matrix_mask, zero_mask_matrix), 0, dtype=np.int32)
        #     # matrix_attention_mask = block_diag(matrix_mask, np.zeros((difference,difference)))
        #     del matrix_mask
        #     del cat_matrix
        #     del one_mask_matrix
        #     del zero_mask_matrix
        # else:
        #     matrix_attention_mask = [1] * len(required_input) + [0] * difference
        # del previous_counter
        # del index
        # del id
        # # gc.collect()
        ...
        #     # if token=="<sep>" then append count for masking ids
        #     if id==32100:
        #         if matrix_attention_mask.size==0:
        #             matrix_attention_mask = np.zeros((max_length,max_length), dtype=np.int32)
        #             matrix_attention_mask[0:index - previous_sep_index,0:len(required_input)] = 1
        #         else:
        #             matrix_attention_mask[row_index:index - previous_sep_index, previous_sep_index+1:index+1] = 1
        #         previous_sep_index = index
        #         row_index += previous_sep_index
        # if previous_sep_index >= 0:
        #     matrix_attention_mask[row_index:index - previous_sep_index, previous_sep_index+1:len(required_input)] = 1
        # else:
        #     matrix_attention_mask = [1] * len(required_input) + [0] * difference
        ...
        for index, id in enumerate(required_input):
            ...
            if id==32100:
                if matrix_attention_mask.size==0:
                    ...
                    matrix_attention_mask[row_index+1:index+1, previous_sep_index+1:index+1] = 1
                previous_sep_index = index
                row_index = previous_sep_index
        if previous_sep_index >= 0:
            matrix_attention_mask[row_index+1:index+1, previous_sep_index+1:len(required_input)] = 1
        else:
            matrix_attention_mask = [1] * len(required_input) + [0] * difference
        return matrix_attention_mask
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == "right":
                if return_attention_mask:
                    ...
                    #    0, 0, 1, 1, 1, .....],
                    #    ....,
                    # self._create_attention_mask(difference, max_length, required_input)
                    # matrix_mask = np.zeros(0)
                    # index - previous_counter - 1 = 0
                    # previous_counter = 0
                    # for i, id in enumerate(required_input):
                    #     # if token=="<sep>" then append count for masking ids
                    #     if id==32100:
                    #         if matrix_mask.size==0:
                    #             matrix_mask = np.ones((index - previous_counter - 1+1,len(required_input)))
                    #         else:
                    #             # concatenate (previous_mask, current_mask, zero_mask) Size: length_mask_row x length_mask_column
                    #             matrix_mask = np.concatenate((matrix_mask, np.concatenate((np.zeros((index - previous_counter - 1+1,previous_counter+1)),np.ones((index - previous_counter - 1+1,index - previous_counter - 1+1)),np.zeros((index - previous_counter - 1+1, (len(required_input)-previous_counter-index - previous_counter - 1-2)))),1)),0)
                    #         previous_counter += index - previous_counter - 1
                    #         index - previous_counter - 1 = 0
                    #     else:
                    #         index - previous_counter - 1 += 1
                    # zero_mask_matrix = (np.zeros((index - previous_counter - 1,previous_counter+1)) if previous_counter!=0 else np.zeros((index - previous_counter - 1,previous_counter)))
                    # one_mask_matrix = (np.ones((index - previous_counter - 1,(len(required_input)-previous_counter-1))) if previous_counter!=0 else np.ones((index - previous_counter - 1,len(required_input))))
                    # cat_matrix = np.concatenate((zero_mask_matrix, one_mask_matrix),1 )
                    # matrix_mask = (np.concatenate((matrix_mask, cat_matrix),0) if previous_counter!=0 else cat_matrix)
                    # matrix_attention_mask = block_diag(matrix_mask, np.zeros((difference,difference)))
                    # encoded_inputs["attention_mask"] = matrix_attention_mask
                    encoded_inputs["attention_mask"] = self._create_attention_mask(difference, max_length, required_input)
